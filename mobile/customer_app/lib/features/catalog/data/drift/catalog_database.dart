...
class CatalogDatabase {
  ...
  Future<List<Product>> searchProducts(String storeId, String query, {String? categoryId, int page = 1, int pageSize = 20}) async {
    final db = await database;
  ...
  Future<List<Category>> getCategories(String chainId) async {
    ...
    // Map parent-children category tree
  ...
  Future<void> batchWriteSyncPage({
    ...
    await db.transaction((txn) async {
      ...
      // Delete soft-deleted products in same transaction batch
      ...
      // Upsert products in same transaction batch
      ...
      // Update sync metadata in same transaction batch
      batch.insert(
        'catalog_sync_meta',
        {
          'store_id': storeId,
