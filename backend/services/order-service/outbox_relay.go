...
func (r *OutboxRelay) processBatch() {
	...
	for _, ev := range events {
		...
		if pubErr == nil {
			_, updateErr := tx.ExecContext(ctx, `UPDATE order_creation_outbox SET published_at = CURRENT_TIMESTAMP WHERE id = $1`, ev.id)
			if updateErr != nil {
			...
			_, updateErr := tx.ExecContext(ctx, `UPDATE order_creation_outbox SET retry_count = $1 WHERE id = $2`, newRetry, ev.id)
			if updateErr != nil {
